...
xlabel('Sample');
ylabel('Amplitude');
axis([-inf inf -max(abs(original_matrix(:, num_of_ind)))*0.8-0.2 max(abs(original_matrix(:, num_of_ind)))*0.8+0.2]);
num_of_ind = 240;
subplot(2, 2, 4);
plot(original_matrix(:, num_of_ind));
title(['Frame ' num2str(num_of_ind)]);
...
xlabel('Sample');
ylabel('Amplitude');
axis([-inf inf -max(abs(original_matrix(:, num_of_ind)))-1 max(abs(original_matrix(:, num_of_ind)))+1]);
subplot(2, 2, 2);
plot(hamming_matrix(:, num_of_ind));
title(['Frame ' num2str(num_of_ind)]);
xlabel('Sample');
ylabel('Amplitude');
axis([-inf inf -max(abs(hamming_matrix(:, num_of_ind)))-1 max(abs(hamming_matrix(:, num_of_ind)))+1]);
subplot(2, 2, 3);
...
subplot(5, 1, 5);
plot((1:length(y)) / shiftsize, y);
axis ([-inf inf -max(abs(y))-1 max(abs(y))+1]);
hold on;
plot([begin_ind, begin_ind], [-max(abs(y)), max(abs(y))], 'r');
plot([end_ind, end_ind], [-max(abs(y)), max(abs(y))], 'g');
title('End point detection');
